...
const AdCampaignsScreen = () => {
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel('my-campaigns')
            .on(
                'postgres_changes',
                { event: '*', schema: 'public', table: 'campaigns', filter: `partnerId = eq.${user.id} ` },
                (payload) => {
                    fetchCampaigns(); // Simple refetch on change
                }
            )
            .subscribe();
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) => (
                        ...
                        >
                            <Image source={{ uri: item.bannerUrl }} style={styles.bannerImg} />
                            <View style={styles.cardInfo}>
                                <View style={styles.cardHeader}>
                                    <View>
                                        <Text style={[styles.campaignName, { color: theme.text }]}>{item.name}</Text>
                                        <Text style={[styles.campaignType, { color: theme.subtext }]}>{item.targetType}: {item.targetValue}</Text>
                                    </View>
                                    <View style={[styles.statusBadge, { backgroundColor: item.isActive ? theme.success + '15' : theme.subtext + '15' }]}>
                                        <Text style={[styles.statusText, { color: item.isActive ? theme.success : theme.subtext }]}>{item.status}</Text>
                                    </View>
                                </View>
                                <View style={styles.statsRow}>
                                    ...
                                    <Text style={[styles.dateText, { color: theme.subtext }]}>{item.startDate} to {item.endDate}</Text>
                                </View>
                            </View>
                        </Animated.View>
                    )}
