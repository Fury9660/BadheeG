...
const InventoryScreen = () => {
    ...
    useFocusEffect(
        useCallback(() => {
            ...
            const channel = supabase.channel('inventory-list')
                .on('postgres_changes', { event: '*', schema: 'public', table: 'products', filter: `partner_id=eq.${user.id}` }, fetchProducts)
                .subscribe();
    ...
    const toggleStock = async (id: string, currentStatus: boolean) => {
        ...
        // Optimistic update
        setProducts(prev => prev.map(p => p.id === id ? { ...p, inStock: !currentStatus } : p));

        try {
            const { error } = await supabase.from('products').update({ inStock: !currentStatus }).eq('id', id);
            ...
            <View style={styles.imageContainer}>
                ...
                <View style={styles.stockToggle}>
                    <Switch
                        ...
                        onValueChange={() => toggleStock(item.id, item.inStock !== false)}
