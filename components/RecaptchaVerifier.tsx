...
import { ActivityIndicator, Alert, Modal, Platform, SafeAreaView, StyleSheet, Text, TextStyle, TouchableOpacity, View, ViewStyle } from 'react-native';

// Lazy load WebView to avoid web bundler issues if possible, or just strict guard
let WebView: any;
if (Platform.OS !== 'web') {
...
const RecaptchaVerifier = forwardRef<RecaptchaVerifierHandle, RecaptchaVerifierProps>((props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        show: () => {
            if (Platform.OS === 'web') {
                // On web, the verifier is usually always present on screen or invisible
                // We don't need to show a modal. The parent component will handle the SDK call.
                return;
            }
            setVisible(true);
            setLoading(true);
    ...
    if (Platform.OS === 'web') {
        // On web, we simply render a container for the Firebase SDK to mount the reCAPTCHA
        // Use standard HTML div to ensure ID is strictly respected
                ...
                <View style={{ flex: 1 }}>
                    {loading && (
                        <View style={styles.loaderContainer}>
                            <ActivityIndicator size="large" color="#3466F6" />
