...
async function test() {
    ...
    try {
        ...
        console.log("\n--- STEP 2: CREATE MANIFEST (LTL JSON PAYLOAD) ---");
        const shortId = "LTJSON" + Date.now().toString().slice(-6);
        ...
        const manifestRes = await axios.post(`https://ltl-clients-api.delhivery.com/manifest`, payload, {
            headers: { 
