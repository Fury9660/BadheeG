            ...
            <View style={styles.content}>
                <View style={[styles.mainCard, { backgroundColor: theme.card }]}>
                    <View style={[styles.header, { marginTop: isSmallDevice ? 20 : 40, marginBottom: isSmallDevice ? 20 : 40 }]}>
                        <Text style={[styles.title, { color: theme.text, fontSize: width < 300 ? 20 : isSmallDevice ? 24 : 28 }]}>Seller Verification</Text>
                        <Text style={[styles.subtitle, { color: theme.subtext, fontSize: width < 300 ? 12 : isSmallDevice ? 14 : 16 }]}>Enter code sent to +91 {phoneNumber}</Text>
                    </View>
                    <View style={styles.otpBoxesContainer}>
                        {otp.map((digit, index) => (
                            <TextInput
                                ...
                                style={[styles.otpBox, { backgroundColor: theme.card, color: theme.text, borderColor: theme.border, borderWidth: 1.5, height: width < 300 ? 50 : 60, fontSize: width < 300 ? 20 : 24, padding: 0, ...Platform.select({ web: { outlineStyle: 'none' } }) } as any]}
                                maxLength={1}
                    ...
                    <TouchableOpacity
                        style={[styles.button, { backgroundColor: '#000000', opacity: otp.join('').length === 6 ? 1 : 0.6, height: width < 300 ? 50 : 60 }]}
                        onPress={handleVerify}
                        disabled={loading || otp.join('').length < 6}
                    >
                        {loading ? <ActivityIndicator color="#fff" /> : <Text style={[styles.buttonText, { fontSize: width < 300 ? 16 : 18 }]}>Verify & Continue</Text>}
                    </TouchableOpacity>
